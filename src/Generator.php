...
class Generator {
     ...
     * @param integer $id            ID of the shortlink
     * @param integer $initialLength Initial length of the shortlink
    ...
    public function __construct($url = '', $initialLength = 4, $alphabet = []) {
        ...
        if ( $initialLength > 0 ) $this->initialLength = $initialLength;
        ...
        if ( $this->initialLength > 1 ) {
            ...
            for ( $i = 1; $i < $this->initialLength; ++$i ) $this->skip += pow($this->alphabetSize, $i);
    ...
    public function get($id = 1) {
        ...
        $position = 0;
        for ( $i = 1; $i < $amountOfLetters; ++$i ) $position += pow($this->alphabetSize, $i);
        ...
        for ( $i = $nextLetter; $i >= 1; $i-- ) {
            ...
            else $shortlink .= $this->alphabet[($index - $iteration * ($letter - 1)) - 1];   
